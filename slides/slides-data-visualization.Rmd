...
```{r remove_axis_titles_theme_minimal}
third_grade_math_proficiency %>% 
  filter(district == "Portland SD 1J") %>% 
  ggplot(aes(x = year, y = percent_proficient,
             group = school)) +
...
```{r fig.height = 5, fig.width = 11, ref.label = "remove_axis_titles_theme_minimal", echo = FALSE, include = TRUE, eval = TRUE}
```
...
highlight_school %>% 
  select(school, percent_proficient_display)
```
